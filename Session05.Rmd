...
intercept <- coef(reg1)[1]
slope <- coef(reg1)[2]
ggplot(Prestige, aes(x=education, y=prestige)) + geom_point() + geom_abline(intercept = intercept, slope = slope)
```
...
```{r}
### Plot the data, 
reg_chol<-lm(CHOL ~ rs12140539, data = data)
summary(reg_chol)
```
```{r}
ggplot(data$CHOL, aes(x = rs6669795, y = CHOL)) + geom_point()
```

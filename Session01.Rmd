...
library(dplyr)
library(readr)
data <- read_csv('/Users/matthewkirschenbaum/Desktop/Bioinformatics in R/BreastCancerData-Class01 2.csv')
```
...
```{r}
data %>%
  summarize(average_exp = mean(FPKM), max_exp = max(FPKM), min_exp = min(FPKM))
```
...
data %>%
  filter(gene == 'BRCA1')  %>%
  summarize(average_FPKM = mean(FPKM))### pipe your commands here
```

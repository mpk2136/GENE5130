...
counts1 <- read.csv('/Users/matthewkirschenbaum/Desktop/Bioinformatics in R/airway_scaledcounts.csv', row.names = 1, header = TRUE)
metadata1 <- read.csv('/Users/matthewkirschenbaum/Desktop/Bioinformatics in R/airway_metadata.csv', row.names = 1, header = TRUE)
```
...
### How many genes are retained? 19038 genes retined
i.e. how many passed the filtering step? 19656 genes excluded

**Answer Here**: 

Set a factor level:
#### Important, DESeq2 will need a reference (think about it as the baseline, wildtype, untreated, control, etc...)
It will normally take the first one alphabetical, but to make sure you can use the relevel function! Look at this example, edit
